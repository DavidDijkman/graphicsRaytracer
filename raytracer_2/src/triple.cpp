...
Triple Triple::rotate(Triple const &t) const {
    double y1 = y * cos(t.x) - z * sin(t.x);
    double z1 = y * sin(t.x) + z * cos(t.x);
    double x1 = x;

    // Rotation around y axis
    double x2 = x1 * cos(t.y) + z1 * sin(t.y);
    double z2 = -x1 * sin(t.y) + z1 * cos(t.y);
    double y2 = y1;

    // Rotation around z axis
    double x3 = x2 * cos(t.z) - y2 * sin(t.z);
    double y3 = x2 * sin(t.z) + y2 * cos(t.z);
    double z3 = z2;
